...
bgCarousel.prototype={
	...
	init:function($, slidesHTML){
		...
		var $controls =  $('<img id="firstb" class="navbutton" src="'+controlpaths[1]+'" data-dir="first" style="position:absolute; z-index:5; cursor:pointer; ' + (orientation=='v'? 'bottom:8px; left:46%' : 'top:46%; right:8px;') + '" />'
			+ '<img id="secondb" class="navbutton" src="'+controlpaths[0]+'" data-dir="second" style="position:absolute;z-index:5; cursor:pointer; ' + (orientation=='v'? 'top:8px; left:45%' : 'top:45%; left:8px;') +'" />'
			+ '<img id="thirdb" class="navbutton" src="'+controlpaths[0]+'" data-dir="third" style="position:absolute;z-index:5; cursor:pointer; ' + (orientation=='v'? 'top:8px; left:45%' : 'top:45%; left:8px;') +'" />'
		)
		.css({opacity:0})
		.click(function(){
			var keyword = this.getAttribute('data-dir')
			if(keyword=="first")
				slideshow.slide(0, "right")	
			else if(keyword=="second")
				slideshow.slide(1, "right")
			else
				slideshow.slide(2, "right")
